	...
	<script>
		...
		$(function(){
		    $("#findSubmit").click(function(){
		        let checkID = /^[\w.-]{6,30}@[a-zA-Z\d.-]+\.[a-zA-Z]{2,}$/;
		        let email = $("#findEmailInput").val();
		        if(email==""){
		            alert("이메일을 입력하세요.");
		        } else if(!checkID.test(email)){
		            alert("아이디는 6~30자의 영문 소문자, 숫자를 포함한 이메일 형식만 사용 가능합니다.");
		        } else {
		            alert("123");
		            find_password(email);
		        }
				...
				$.ajax({
					...
					success: function(check){
						...
						if (check=true){
							...
							alert("이메일 형식이 잘못되었거나, 존재하지 않는 아이디 입니다.");
						}
					}
				});
			}
...
<body>
	<header class="with-header main fadein">
				...
				<nav class="gnb menu">
					<ul id="Gnb">
						...
						<li class="gnbList">
							...
							<ul class="gnbDepth productMenu">
								<li>
									<a title="요금제" href="">요금제</a>
								</li>
								<li>
									<a title="구축형 문의" href="">구축형 문의</a>
								</li>
							</ul>
						</li>
						<li class="gnbList">
		...
		<div class="findPassword" style="diplay:block;">
			<div class="password-text">
				비밀번호 찾기
			</div>
			<div class="password-sub-text">
				기존에 가입하신 이메일을 입력하시면, 임시 비밀번호를 발송해드립니다.
			</div>
			<div class="password-detect">
				<input id="findEmailInput" type="text" placeholder="이메일">
				<div style="height:40px;"></div>
			</div>
			
			<a id="findSubmit" href="" class="password-button">임시 비밀번호 이메일 받기</a><!-- 메일전송으로 바꿔야함 -->
		</div>
